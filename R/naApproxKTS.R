...
function (){
      ...
      if (length(nasInSelTs) == 0) {
        ...
        for (ppp in 1:peri){
          ...
          if(length(gg) > 0){
            ...
            filledTS.i$value[gg] <- zoo::na.approx(filledTS.i$value, 
                                                   xout = gg, 
                                                   na.rm = FALSE)
        ...
        assign(paste0(selTsName, "_", selGapName, "_li"), 
  ...
  showPANnaappr <- function() {
    ...
    createTsRb()
    createEntry(labTitle = "Period", 
                textVariableName = "peri",defaultVal = "1")
