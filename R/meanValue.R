    ...
    if (is.na(selTsName)) {
      ...
      if (length(nasInSelTs) == 0) {
        ...
        fillGapsMV <- function(gaps, TS, maxIter, widthAround, 
                               lPeriod, minPoints) {
          getMedianVal <- function(NAInd, TS, widthAround, 
                                   lPeriod, minPoints) {
            lTS <- length(TS$value)
            eqP <- seq(NAInd - widthAround, NAInd + widthAround,lPeriod)
            eqP <- eqP[eqP > 0 & eqP <= lTS]
            
            anaobi1 <- is.finite(selTs$value[eqP[which(eqP < NAInd)]])
            anaobi2 <- is.finite(selTs$value[eqP[which(eqP > NAInd)]])
            ...
            if (pointsBefore < minPoints | pointsAfter < minPoints) {
              ...
              stats::median(TS$value[eqP], na.rm = TRUE)
            }
          }
          getMeanVal <- function(NAInd, TS, widthAround, lPeriod, minPoints) {
            lTS <- length(TS$value)
            eqP <- seq(NAInd - widthAround, NAInd + widthAround, 
                               lPeriod)
            eqP <- eqP[eqP > 0 & eqP <= lTS]
            
            lovfl1 <- eqP[which(eqP < NAInd)]
            lovfl2 <- eqP[which(eqP > NAInd)]
            pointsBefore <- which(is.finite(selTs$value[lovfl1]))
            pointsAfter <- which(is.finite(selTs$value[lovfl2]))
            pointsBefore <- length(pointsBefore)
            pointsAfter <- length(pointsAfter)
            if (pointsBefore < minPoints | pointsAfter < minPoints) {
              NA
            } else {
              mean(TS$value[eqP], na.rm = TRUE)
            }
          }
          
          cont <- 1
          remainingGaps <- length(gaps)
          NAsIndices <- which(is.na(TS$value[gaps]))
          while (cont <= maxIter & length(NAsIndices) != 0) {
            if (statistic == "mean") {
              replacements <- sapply(gaps, getMeanVal, minPoints = minPoints, 
                                     TS = TS, widthAround = widthAround, 
                                     lPeriod = lPeriod)
            } else {
              replacements <- sapply(gaps, getMedianVal, minPoints = minPoints, 
                                     TS = TS, widthAround = widthAround, 
                                     lPeriod = lPeriod)
            }
            replacements[which(is.finite(replacements) == FALSE)] <- NA
            TS$value[gaps] <- replacements
            gaps <- intersect(which(is.na(TS$value)), gaps)
            remainingGaps <- c(remainingGaps, length(gaps))
            NAsIndices <- which(is.na(TS$value[gaps]))
            cont <- cont + 1
            rm(replacements)
          }
          resultMV <- list(filled = TS, gaps = gaps, 
                           iterations = cont, advances = remainingGaps)
          rownames(resultMV$filled) <- NULL
          resultMV
        }
        widthAround <- halfNPeriods * lPeriod
        resMeVal <- fillGapsMV(gaps = selGap$gaps, TS = selTs, 
                               maxIter = maxIter, 
                               widthAround = widthAround, lPeriod = lPeriod, 
                               minPoints = minPoints)
        ...
        txtParam <- c(paste("Period(sec):", lPeriod * sampPerSec), 
                      ...
                      paste("Minimum number of observations to each side:", 
                            minPoints), 
                      paste("Filled NAs per iteration:", 
